...
import { getArchivedProperties } from "../../features/realEstateOwner/realEstateOwnerSlice";
...
const Homepage = () => {
  ...
  const { allRealEstate, isLoading, numberOfPages ,archivedRealEstates } = useSelector(
  ...
  useEffect(() => {
    dispatch(getArchivedProperties({ page }));
  }, [dispatch, page]);
    ...
    <>
      <div className="flex flex-col mt-8 mb-12 md:items-start md:ml-10">
        <h3 className="my-4 font-heading font-bold text-center">
          Your {archivedRealEstates?.length > 1 ? "Properties" : "Property"}
        </h3>
        {isLoading ? (
